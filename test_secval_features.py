...
class SimpleValidator(BaseValidator):
    name: str = Field(str, no_empty=True, max_length=50)
    age: int = Field(int, min_value=0, max_value=150)
    score: float = Field(float, min_value=0.0, max_value=100.0)
    active: bool = Field(bool)
...
class ProductValidator(BaseValidator):
    ...
    category: str = Field(str, choices=['electronics', 'clothing', 'food', 'books'])
...
def test_string_sanitizer():
    ...
    test("is_safe() returns True for safe", StringSanitizer.is_safe("Hello World") == True)
    test("is_safe() returns False for malicious", StringSanitizer.is_safe("<script>bad</script>") == False)
...
def test_email_validator():
    ...
    # is_valid works
    test("is_valid() for valid email", EmailValidator.is_valid("test@example.com") == True)
...
def test_pattern_validation():
    ...
    p = ProductValidator(name="Laptop", sku="ABC-1234", price=999.99, category="electronics")
    test("Valid pattern passes", p.sku == "ABC-1234")
    
    # Invalid pattern rejected
    try:
        ProductValidator(name="Phone", sku="invalid", price=499.99, category="electronics")
...
def test_nested_validators():
    ...
    test("Valid nested data passes", u.address.city == "New York")
    test("Nested default works", u.address.country == "USA")
    ...
    d = u.dict()
    test("Nested in dict()", isinstance(d["address"], dict) and d["address"]["city"] == "New York")
...
def test_error_handling():
    ...
    try:
        ...
        errors = e.errors()
        has_structure = all("loc" in err and "type" in err and "msg" in err for err in errors)
        test("Error has loc, type, msg", has_structure)
    
    # Missing field type
    try:
        SimpleValidator(age=30, score=50.0, active=True)
    except ValidationError as e:
        errors = e.errors()
        name_errors = [err for err in errors if "name" in str(err["loc"])]
        test("Missing field type is 'missing'", len(name_errors) > 0 and name_errors[0]["type"] == "missing")
    
    # Extra fields rejected
    try:
        SimpleValidator(name="John", age=30, score=50.0, active=True, extra="field")
    except ValidationError as e:
        errors = e.errors()
...
def test_list_validation():
    ...
    # Non-list rejected
    try:
        ListValidator(items="not-a-list")
...
def test_optional_fields():
    ...
    class OptionalValidator(BaseValidator):
        required_field: str = Field(str)
        optional_field: str = Field(str, required=False)
        optional_with_default: str = Field(str, required=False, default="default_value")
